...
from xray.api.stats_service_pb2 import GetStatsRequest, QueryStatsRequest
...
class XrayStatsService:
    ...
    def get_client_stats(self, email: str) -> dict:
        ...
        try:
            # Запрос uplink
            uplink_req = GetStatsRequest(name=f"user>>>{email}>>>traffic>>>uplink", reset=False)
            uplink_resp = self.stub.GetStats(uplink_req)
            upload = uplink_resp.stat.value if uplink_resp.stat else 0

            # Запрос downlink
            downlink_req = GetStatsRequest(name=f"user>>>{email}>>>traffic>>>downlink", reset=False)
            downlink_resp = self.stub.GetStats(downlink_req)
